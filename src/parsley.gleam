...
fn reverse_match_list(state: ParserState(List(a))) -> ParserState(List(a)) {
...
pub fn map(parser: Parser(a), transformer: fn(a) -> b) -> Parser(b) {
  fn(input: String) -> ParserResult(b) {
    case parser(input) {
      Ok(state) ->
        Ok(ParserState(match: transformer(state.match), rest: state.rest))
      Error(m) -> Error(m)
    }
  }
}
...
pub fn many(parser: Parser(a)) -> Parser(List(a)) {
  fn(input: String) -> ParserResult(List(a)) {
    ...
    Ok(do_many_parse(parser, initial_state))
  }
}
...
pub fn sequence(parsers: List(Parser(a))) -> Parser(List(a)) {
  fn(input: String) -> ParserResult(List(a)) {
    list.fold(parsers, Ok(ParserState([], input)), do_sequence_parse)
    |> result.map(reverse_match_list)
  }
}
...
fn do_sequence_parse(
  ...
  case previous_state {
    Ok(state) -> {
      case parser(state.rest) {
        Ok(new_state) ->
          Ok(ParserState([new_state.match, ..state.match], new_state.rest))
        Error(msg) -> Error(msg)
      }
    }
    Error(msg) -> Error(msg)
...
pub fn alpha(input: String) -> ParserResult(String) {
  case regexp.from_string("^[a-zA-Z]+") {
    ...
    Error(CompileError(detail, _)) -> Error(detail)
  }
}
...
pub fn int(input: String) -> ParserResult(Int) {
  input |> bind(do_int_parse, do_int_conversion)
}
...
fn do_int_conversion(state: ParserState(String)) -> ParserResult(Int) {
  case int.parse(state.match) {
    Ok(n) -> Ok(ParserState(match: n, rest: state.rest))
    Error(_) -> handle_error("a number", state.rest)
  }
}

pub fn float(input: String) -> ParserResult(Float) {
  input |> bind(do_float_parse, do_float_conversion)
}
...
pub fn until(predicate: fn(String) -> Bool) -> Parser(String) {
  fn(input: String) -> ParserResult(String) {
    Ok(do_parse_until(ParserState("", input), predicate))
  }
}
