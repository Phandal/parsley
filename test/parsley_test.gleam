...
import parsley.{
  type ParserResult, type ParserState, ParseError, ParseOk, ParserState,
}
...
fn upcase_result(state: ParserState(String)) -> ParserResult(String) {
  case state.match {
    "fail" -> ParseError("upcase_result expected failure")
    _ ->
      ParseOk(ParserState(
...
pub fn string_test() {
  ...
  x_parser("xkcd")
  |> should.equal(ParseOk(ParserState("x", "kcd")))
}

pub fn string_does_not_match_test() {
  let x_parser = parsley.string("x")
  x_parser("kcd")
  |> should.equal(ParseError("expected x but got 'kcd...'"))
}
...
pub fn string_matches_case_test() {
  ...
  |> should.equal(ParseOk(ParserState("Hello", " World!")))
}

pub fn alpha_test() {
  parsley.alpha("aduoiy123kjl")
  |> should.equal(ParseOk(ParserState("aduoiy", "123kjl")))
}

pub fn alpha_with_digits_test() {
  parsley.alpha("23helpme")
  |> should.equal(ParseOk(ParserState("", "23helpme")))
}

pub fn alpha_one_test() {
  parsley.alpha_one("abcd4567ghi")
  |> should.equal(ParseOk(ParserState("abcd", "4567ghi")))
}
...
pub fn digit_test() {
  ...
  |> should.equal(ParseOk(ParserState("1234", "abcd")))
}

pub fn digit_with_characters_test() {
  parsley.digit("abcd1234")
  |> should.equal(ParseOk(ParserState("", "abcd1234")))
}
...
pub fn map_uppercase_test() {
  ...
  |> should.equal(ParseOk(ParserState("GOODBYE", " mars")))
}
...
pub fn many_test() {
  ...
  |> should.equal(ParseOk(ParserState(["abc", "abc"], "def")))
}
...
pub fn many_empty_test() {
  ...
  |> should.equal(ParseOk(ParserState([], "defabc")))
}
...
pub fn many_one_empty_test() {
  ...
  many_empty_parser("defabc")
  |> should.equal(ParseError("expected at least one match but got '[]...'"))
}
...
pub fn sequence_test() {
  ...
  |> should.equal(ParseOk(ParserState(["abc", "def"], "")))
}
...
pub fn sequence_error_test() {
  ...
  |> should.equal(ParseError("expected def but got 'gdef...'"))
}
...
pub fn recursive_sequence_and_map_test() {
  ...
  |> should.equal(ParseOk(ParserState([["abc", "def"], ["GHI", "jkl"]], "")))
}
...
pub fn choice_single_test() {
  ...
  |> should.equal(ParseOk(ParserState("123", "")))
}
...
pub fn choice_multiple_test() {
  ...
  |> should.equal(ParseOk(ParserState("abc", "")))
}
...
pub fn choice_none_test() {
  ...
  |> should.equal(ParseError("expected one choice to match but got '123...'"))
}

pub fn chain_test() {
  let chain_parser = parsley.chain(parsley.string("abc"), upcase_result)
...
pub fn chain_error_test() {
  let chain_parser = parsley.chain(parsley.string("fail"), upcase_result)

  chain_parser("fail")
  |> should.equal(ParseError("upcase_result expected failure"))
}

pub fn int_positive_test() {
  parsley.int("56")
  |> should.equal(ParseOk(ParserState(56, "")))
}
...
pub fn int_character_test() {
  parsley.int("abc")
  |> should.equal(ParseError("expected a number but got 'abc...'"))
}

pub fn float_positive_test() {
  parsley.float("12.0")
  |> should.equal(ParseOk(ParserState(12.0, "")))
}
...
pub fn float_character_test() {
  parsley.float("abc")
  |> should.equal(ParseError("expected a float but got 'abc...'"))
}

pub fn alpha_digit_test() {
  parsley.alpha_digit("abc123..")
  |> should.equal(ParseOk(ParserState("abc123", "..")))
}
